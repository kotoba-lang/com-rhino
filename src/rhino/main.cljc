(ns rhino.main
  "Kotodama WASM entrypoint for the Rhino clean-room actor (L4) — Clojure port.
  ...
  facts (`rhino.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"rhino.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rhino-compat" :tier tier :entities entities} 200])
